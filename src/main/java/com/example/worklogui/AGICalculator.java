...
import com.example.worklogui.dto.AGICalculationRequest;
...
public class AGICalculator {
    ...
    public static class AGIResult {
        ...
        // Reduced parameter constructor using builder pattern
        public AGIResult(AGICalculationRequest request) {
            this.grossIncome = request.getGrossIncome();
            this.businessExpenses = request.getBusinessExpenses();
            this.netEarnings = request.getNetEarnings();
            this.nese = request.getNese();
            this.selfEmploymentTax = request.getSelfEmploymentTax();
            this.selfEmploymentTaxDeduction = request.getSelfEmploymentTaxDeduction();
            this.adjustedGrossIncome = request.getAdjustedGrossIncome();

            // If data is already monthly, don't divide by 12
            this.monthlySSACountableIncome = request.isMonthlyData() ? request.getNese() : request.getNese() / 12.0;

            this.expensesByCategory = request.getExpensesByCategory();
        }
    }
    ...
    public static AGIResult calculateAGI(List<RegistroTrabalho> registros, List<Bill> bills, boolean isMonthlyData) {
        ...
        // Use builder pattern to reduce parameter count
        AGICalculationRequest request = AGICalculationRequest.builder()
                .grossIncome(grossIncome)
                .businessExpenses(totalBusinessExpenses)
                .netEarnings(netEarnings)
                .nese(nese)
                .selfEmploymentTax(selfEmploymentTax)
                .selfEmploymentTaxDeduction(selfEmploymentTaxDeduction)
                .adjustedGrossIncome(adjustedGrossIncome)
                .expensesByCategory(expensesByCategory)
                .isMonthlyData(isMonthlyData)
                .build();

        return new AGIResult(request);
    }
